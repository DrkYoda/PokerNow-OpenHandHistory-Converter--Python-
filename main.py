...
goal of this program is to process Poker Now (https://www.pokernow.club/) hand
history csv files and convert to JSON format that matches the standardized open
hand History format specified by PokerTracker and documented here:
...
10-06-22 v 1.0.3
    ...
    - Made improvements to the way the program selects lines that should be
      ignored.
    - Made improvements to the logging feature.
        - Code performance will be logged.
        - Lines that don't get processed will be logged in order to add them
          to the ignore list.
        - It will be logged when the calculated pot amount is not equal to the
          the collected amount.
v 1.0.4
    - Made change to correctly handle dead blinds.
    - Corrected a problem that was causing become progressively slower with
      multiple files.
...
v 1.1.0
    - Fixed issue #13 and added the attribute "is_allin" to action objects that
      have an amount>0.00
    - Fixed issue #14 At showdown the action value is now the valid value
      "Shows Cards"
    - Fixed issue #15 removed the extra comma after cards in the text string
    - Fixed issue #19 game_number attribute is now a unique identifier for a
      hand.
v 1.1.1
    - Fixed issue #23 Unicode characters will be removed from names.
    - Fixed issue #25 Pots are now calculated correctly when the SB posts a
      missed BB
v 1.1.2
    - Fixed issue #26 The blind structure will initially be determined from
      the amounts posted in first hand, but if the blind structure is changed
      after the first hand the structure will be determined from the metadata
      after the first hand.
v 1.1.3
    - Fixed issue #30 If the information in the last hand is incomplete then it
      will not be parsed an converted into the OHH standard.
v 1.2.0
    - Fixed issue #10 The name-map data model is now handled programmatically
      so when an aliase or device ID is not in the data model the program will
      require input from the user and update the data model before continuing.
    - Fixed issue #31 If the hero is not dealt into the hand will have the
      flags value "Observed"
v 1.2.1
    - Fixed issue #32 After a Poker Now csv file is parsed the file is moved to
      an archive folder
    - Improved how the name-map data model is handled programmatically.
v 1.2.2
    - Fixed issue #38 If the hero name is an empty screen, the user will be
      propted to enter a name
v 1.2.3
    - Fixed issue #43
v 1.2.4
    - Fixed issue #45
v 1.3.0
    - Fixed issue #39
        - Changes the regex object usage from using them as the first argument
...
the hands dictionary
    - structure
        ...
        - GAME_TYPE: string - The game type (Texas Hold'em, Omaha High, Omaha
                              Hi/Lo 8 or Better)
...
the tables dictionary
    - structure
        ...
        - LATEST: datetime - the latest time stamp for a hand processed for
                             this table
...
def get_config(path: Path) -> ConfigParser:
    """Get the config object from the .ini file at path.
...
def load_name_map(file: Path):
    """Open aliase->name map file (aliase-name_map.json) and parse it.
...
def switch_key_and_values(
    name_map: dict[str, dict[str, list[str]]], key_txt: str,
):
    """Players can choose a different alias every time they sit at the table.

    therefore, it is necissary to map the players real name to the aliases they
    have chosen. The information needed to create the aliase->name map is
    recorded in the file aliase-name_map.json where each player name has an
    array of aliases. The data will be parsed into a dictionary of lists where
    the keys are the names and the values are lists of aliases. The
    aliase->name map is created by flattening the alias lists and switching
...
# Check if hero_name is an empty string, if True then prompt the user to input
# a name for the hero and save the name to config.ini
...
# Compile regular expressions for matching to identifiable strings in the
# hand history
...
start_regex = re.compile(
    r'-- starting hand #(?P<hand_number>\d+).+\((?P<bet_type>\w*\s*Limit) '
    r'(?P<game_type>.+)\) \((dealer: \"(?P<player>.+?) @ (?P<device_id>[-\w]+)'
    r'\"|dead button)\) --',
)
...
seats_regex = re.compile(
    r' #(?P<seat>\d+) \"(?P<player>.+?) @ (?P<device_id>[-\w]+)\" '
    r'\((?P<amount>\d+\.\d{2})\)',
)
...
non_bet_action_regex = re.compile(
    r'\"(?P<player>.+?) @ (?P<device_id>[-\w]+)\" (?P<player_action>\w+'
    r'(?![ a-z]+\d+\.\d{2}))'
)
...
for poker_now_file in csv_file_list:
    ...
    if table_name_match is not None:
        ...
        for game_number, hand in hands.items():
            ...
            # initialize the OHH JSON populating as many fields as possible and
            # initializing arrays.
            ohh = {
                SPEC_VERSION: spec_version,
                SITE_NAME: site_name,
            ...
            for line in hand_text.strip().splitlines(False):
                # The text match to look for a seated player and see their
                ...
                if round_marker is not None:
                    ...
                    if label == PLAYER_STACKS:
                        ...
                        # make a clean one increment round number and reset
                        # action number
                ...
                if winner is not None:
                    ...
                    if player_id not in pot_obj[pot_number][PLAYER_WINS]:
                ...
                # that will have made it this far will indicat that all players
                # approved.
